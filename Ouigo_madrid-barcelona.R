...
destination$sendKeysToElement(list("Barcelona - Sants"))
Sys.sleep(2)
...
for (option in suggestions) {
  if (grepl("Barcelona - Sants", option$getElementText()[[1]])) {
    option$clickElement()
    break
  }
}
...
#######################################################
#date_7 The date is seven days later, and all other operations remain the same as for one day later.!!!
...
ouigo <- ouigo |> 
  mutate(
    departure_station = str_replace(departure_station, "\\s*-.*", ""),  # 删除第一个 "-" 及其后面内容
    arrival_station = str_replace(arrival_station, "\\s*-.*", "")  # 同样操作
  ) |> 
  mutate(price = str_remove(price, "€") %>% as.numeric()) |> 
  rename(duration_raw = duration) |>  
  mutate(duration_min_clean = as.numeric(str_extract(duration_raw, "\\d+(?=h)")) * 60 +  
