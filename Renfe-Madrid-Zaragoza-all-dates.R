...
arrival_station <- "Zaragoza"  #Stores the departure (Madrid) and arrival (Zaragoza) station names as variables.
...
destination_input$sendKeysToElement(list("Zaragoza"))  #Similar to the departure station, but enters "Zaragoza".
Sys.sleep(2)

suggestions <- remDr$findElements(using = "css selector", "ul li")
for (option in suggestions) {
  if (grepl("ZARAGOZA \\(TODAS\\)", option$getElementText()[[1]])) {
    option$clickElement()
    break
  }
}
Sys.sleep(1) #Finds and selects "ZARAGOZA \\(TODAS\\)" from the dropdown.
...
destination_input$clickElement()
destination_input$clearElement()
destination_input$sendKeysToElement(list("Zaragoza"))
Sys.sleep(2)
